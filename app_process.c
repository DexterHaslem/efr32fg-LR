...
// 16 size, 256 fifo works, once fix size packet is called
// 8, 1 size 256 fifo work too
// 128 works, 64, anything lower than 64 barfs
/// Size of RAIL RX/TX FIFO
#define RAIL_FIFO_SIZE (128U)
/// Transmit data length
#define TX_PAYLOAD_LENGTH (1U)
...
/// Transmit packet
static uint8_t out_packet[TX_PAYLOAD_LENGTH] = { 0xDE };
...
void sl_rail_util_on_event(RAIL_Handle_t rail_handle, RAIL_Events_t events)
{
  ...
  if ((events & RAIL_EVENT_TX_STARTED) != 0)
       ...
       if ((events & RAIL_EVENT_RX_FIFO_OVERFLOW ) != 0)
             {
               app_log_warning("got RAIL_EVENT_RX_FIFO_OVERFLOW \r\n");
             }
       if ((events & RAIL_EVENT_TX_PACKET_SENT) != 0)
       {
         app_log_warning("RAIL_EVENT_TX_PACKET_SENT\r\n");
       }

       if ((events & RAIL_EVENT_TX_BLOCKED) != 0)
       {
         app_log_warning("RAIL_EVENT_TX_BLOCKED\r\n");
       }
       ...
       if ((events & RAIL_EVENT_TX_FIFO_ALMOST_EMPTY) != 0)
        {
          app_log_warning("RAIL_EVENT_TX_FIFO_ALMOST_EMPTY\r\n");
        }
...
{
  RAIL_SetFixedLength(rail_handle, TX_PAYLOAD_LENGTH);
